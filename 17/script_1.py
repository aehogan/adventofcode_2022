...
AIR, SIDE_WALL, CORNER_WALL, BOTTOM_WALL, SOLID_ROCK, FALLING_ROCK = range(6)
...
def spawn_rock(Map, rock_counter, shape):
    ...
    rock_pos = np.array([y_start - 3 - shape.shape[0], 3])
...
def test_rock_movement(Map, rock_pos, shape):
    ...
    for y in range(shape.shape[0]):
        for x in range(shape.shape[1]):
            if shape[y, x] != AIR:
                element = Map[rock_pos[0] + y, rock_pos[1] + x]
                if element == SIDE_WALL or element == SOLID_ROCK or element == BOTTOM_WALL or element == CORNER_WALL:
                    can_move = False
    return can_move
...
print_map(0, 8, map_depth - 1000, map_depth)
...
for y in range(map_depth + 1):
    ...
    if y_start != -1:
        break
print(y_start, map_depth - y_start)
