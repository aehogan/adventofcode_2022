...
def simulation(weights, blueprint):
    ...
    max_ore = np.max([blueprint["ore"]["ore"], blueprint["clay"]["ore"], blueprint["obsidian"]["ore"], blueprint["geode"]["ore"]])
...
def genetic_opt(initial_weights, blueprint):
    ...
    for step in range(steps):
        ...
        for i, weight in enumerate(weight_matrix):
            scores[i] = get_score(weight, 200, blueprint)
...
for i, blueprint in enumerate(blueprints):
    ...
    geodes = np.max([simulation(weights, blueprint) for i in range(100) for weights in weight_matrix])
