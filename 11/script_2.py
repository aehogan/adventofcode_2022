...
class Monkey:
    def __init__(self, starting_items, operation, test, if_true, if_false):
        ...
        for item in starting_items:
            self.items.append(sympy.Integer(item))
    ...
    def update(self, monkeys):
        ...
        for old_worry in self.items:
            ...
            if self.operation[0] == "old":
                ...
                new_worry = sympy.Integer(self.operation[0])

            if self.operation[2] == "old":
                operand = old_worry
            else:
                operand = sympy.Integer(self.operation[2])
            ...
            new_worry = sympy.Mod(new_worry, self.mod)

            test = sympy.Mod(new_worry, self.test) == 0
